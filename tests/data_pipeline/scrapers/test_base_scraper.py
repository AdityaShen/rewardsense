...
class TestBaseScraperContextManager:
    ...
    def test_context_manager_closes_session_on_exit(self):
        ...
        # (We can't easily verify this without more mocking,
        # but we can verify the context manager completes)
        assert True  # Context exited without error
...
class TestBaseScraperScrapeAllCards:
    ...
    def test_scrape_all_cards_updates_cards_scraped_stat(self, scraper):
        ...
        scraper.parse_card_listing = MagicMock(return_value=[
            {"name": "Card 1"},
            {"name": "Card 2"}
        ])
